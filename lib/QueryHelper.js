...
class QueryHelper {
  ...
  static resolveAllPromise (res, op, rows) {
    rows = rows.map(row => {
      return QueryHelper.parseAttributes(op.Model, row);
    });
    op.rows = rows;
    QueryHelper.resolvePromise(res, rows, op);
  }

  static resolveFindPromise (res, op, record) {
    QueryHelper.log('1 Record Retrievied', op);
    op.dbObject = QueryHelper.parseAttributes(op.Model, record);
