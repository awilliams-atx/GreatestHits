...
class QueryHelper {
  ...
  static allOptions (Model, op) {
    op = QueryHelper.basicOptions(Model, op);
    op.queryString = `SELECT * FROM ${op.tableName};`;
    return op;
  }
  ...
  static parseAndAddRows (rows, op) {
  ...
  static resolveAllPromise (rows, op) {
    rows = rows.map(row => {
  ...
  static resolveFindPromise (record, op) {
  ...
  static resolveInsertPromise (row, op) {
