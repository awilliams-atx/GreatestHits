...
class QueryHelper {
  ...
  static checkForError (rej, err, op, cb) {
    if (err) {
      op.db.close();
      rej(err);
    } else {
  ...
  static lastInsertRowIdString (op) {
    return `SELECT last_insert_rowid() FROM ${op.tableName} LIMIT 1`;
  }
  ...
  static resolveAllPromise (res, op, Model, rows) {
    rows = rows.map(row => {
      return QueryHelper.parseAttributes(Model, row);
    });
    op.rows = rows;
    QueryHelper.resolvePromise(res, rows, op);
  }

  static resolveFindPromise (res, op, Model, record) {
    QueryHelper.log('1 Record Retrievied', op);
    op.dbObject = QueryHelper.parseAttributes(Model, record);
    QueryHelper.resolvePromise(res, op.dbObject, op);
  }

  static resolveInsertPromise (res, op, row) {
