    ...
    return new Promise((res, rej) => {
      op.db.all(`SELECT * FROM ${op.tableName};`, [], (err, rows) => {
        if (err) {
          ...
          rows = rows.map(row => {
            return new this(QueryHelper.parseAttributes(row));
          });
          op.rows = rows;
    ...
    return new Promise((res, rej) => {
      op.db.get(query, (err, record)  => {
        if (err) {
          ...
          op.dbObject = new this(QueryHelper.parseAttributes(record));
  ...
  static where (op) {
    ...
    return new Promise ((res, rej) => {
      op.db.all(queryString, [], (err, rows) => {
        ...
        if (op.Constructor) {
          rows = rows.map(row => {
            return new op.Constructor(QueryHelper.parseAttributes(row));
          });
        }
        if (err) {
          op.db.close();
