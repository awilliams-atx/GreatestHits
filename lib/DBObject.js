...
class DBObject {
  static insert (op) {
    ...
    if (op.keepOpen === undefined) { op.keepOpen = true; }
    if (!op.dbObject) {
      op.db.close();
  ...
  static find (op) {
    if (!op.dbObject) { throw '::find must be supplied a DBObject'; }
    let id = op.id ? op.id : op.dbObject.attributes.id;
    if (op.db === undefined) { op.db = DBObject.open(); }
    if (op.keepOpen === undefined) { op.keepOpen = false; }
  ...
  insert (op) {
    if (op === undefined) { op = { keepOpen: false }; }
    if (op.keepOpen === undefined) { op.keepOpen = true; }
  ...
  update (op) {
    if (op === undefined) { op = { keepOpen: true }; }
    if (op.keepOpen === undefined) { op.keepOpen = true; }
