...
class DBObject {
  ...
  static find (op) {
    if (!op.Constructor && !op.dbObject) { throw '::find must be supplied with DBObject Constructor or instance'; }
    if (!op.dbObject && !op.tableName) { throw '::find when given Constructor must be supplied with tableName'}
